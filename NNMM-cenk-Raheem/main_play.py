...
#Comments by Cenk
# To run this code do:
# In a terminal run python2 main_play.py
# In an another terminal run telnet localhost 5802
# Then enter a state like: EOOOOOOOMEOEOMOMMMOEOOEO0055
# The game now runs in a local server
...
def main(netname='trained_networks/TFR/TEST-rawest-TFR', team = "white"):
    if (team == 'white'):
...
def write_next_move_to_file(state,TOc,FROMc,REMOVEc):
    text_file = open("QUANTUM_MOVE.txt", "w")
    text_file.write(str(state) + "\n" )
    text_file.write(str(TOc) + "\n")
    text_file.write(str(FROMc) + "\n")
    text_file.write(str(REMOVEc) + "\n")
    text_file.close()
